...
class WorkBenchBotTbVw: UIView,UITableViewDelegate,UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        if indexPath.row != 0 && indexPath.row != self.numberOfRowInSection - 1 {
            ...
            let noteModel = NoteLogicBLL().getNoteModelWithID(id: resultDate.eventModel!.id)
            NoteCreatingBLL.getInstance().showingNoteModel = noteModel
            let itemcon = NoteTimeLineViewController()
            itemcon.hidesBottomBarWhenPushed = true
...
class WorkBenchTBCell: UITableViewCell {
    ...
    override init(style: UITableViewCellStyle, reuseIdentifier: String?) {
        ...
        subTitleLb.snp.makeConstraints { (make) in
            make.centerY.equalTo(dateEnd.snp.centerY)
