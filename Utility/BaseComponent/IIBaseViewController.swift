...
class IIBaseViewController: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        self.navigationItem.leftBarButtonItem =
            UIBarButtonItem(image: UIImage(named: "back.png"),
                            style: UIBarButtonItemStyle.plain, target: self, action: #selector(self.goBack))
        self.navigationController?.navigationBar.titleTextAttributes =
            [NSAttributedStringKey.foregroundColor: APPDelStatic.themeColor]
    }
    ...
    func getBeforeVC() -> UIViewController? {
        for traceVCIndex in 0 ..< self.navigationController!.viewControllers.count {
            if self.navigationController!.viewControllers[traceVCIndex] === self {
                return self.navigationController!.viewControllers[traceVCIndex - 1]
            } else {
                continue
            }
        }
